...
select min(salary) as 최저임금, max(salary) as 최고임금, abs(max(salary) - min(salary)) as '최고임금 - 최저임금' from salaries;
...
-- 예) 2014년 07월 10일
select date_format(min(hire_date), '%Y년 %m월 %d일')from employees a, titles b where a.emp_no = b.emp_no ;
